(function() {
    ...
    window.onload = function() {
        ...
        button.addEventListener("click", function() {
            ...
            $.get("api/text/latest", function(data, status) {
                ...
                var template = _.template('<a href="text/<%- key %>"><p class="latest-entry ellipsize"><%- content %></p></a>');

                var html = [];
                for ( var i = 0; i < data.length; i++) {
                    var entry = data[i];
                    html += template({ key: entry.key, content:entry.content });
                }
